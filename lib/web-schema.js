...
(function () {
    ...
    ,   validateDatetimeLocal:   function (obj, schema, errors) {
            ...
            if (!match) {
                ...
                if (year < 1) errs.push("Year out of range");
                if (month < 1 || month > 12) errs.push("Month out of range");
                if (day < 1 || day > 31) errs.push("Day out of range");
                if (hours < 0 || hours > 23) errs.push("Hours out of range");
                if (minutes < 0 || minutes > 59) errs.push("Minutes out of range");
                if (seconds < 0 || seconds > 59) errs.push("Seconds out of range");
                if (errs.length) {
                    errors.concat(errs);
                    return false;
                }
